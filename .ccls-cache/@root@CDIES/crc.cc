        ...
        try{
          if(vit!=tmp.end()){
            ...
            try{
              ...
              cerr<<"-d选项后面要求必须有一个参数,且不为特殊符号,请仔细检查!"<<endl;
              exit(1);
            }
...
void compile(string file,string flags,string res){
  try{
    ...
    flag+=file;flag+=" -o " ;flag+=res;flag+=flags;
    int index = 0;
    string cmd="g++";
    cmd+=flag;
    ...
    if(status==0){
      cout<<"编译"<<file<<"成功!编译时间为: "<<__TIME__<< "\n编译"<<file<<"文件总用时: "<<(double)programTimes<< "秒"<<"\n运行结果如下:\n\n";
...
void check_empty(string file){
  ifstream tmp(file,ios::in);
  if(!tmp.is_open()){
    cerr<<"编译意外终止,没有"<<file<<"这个文件\n";
    exit(1);
  }
  tmp.close();
}
